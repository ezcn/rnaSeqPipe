...
for (st in sample_types){ 
    ...
    hp<- ggplot(mydheat, aes(x = reorder(ensid, counts), sample) ) +geom_tile(aes(fill=counts) ) +theme_bw() + scale_fill_gradient2(low = "#2a1a5e", high = "#f45905")  +ggtitle( paste( st, " FDR <", FDR.thres, "abs(logFC) >", logFC.thres,  sep=" " ) ) +xlab("") + ylab("")+
theme(legend.box.background=element_blank(),
    ...
    png(paste(outdir, "heat.", st , ".png", sep=""), res=300 ,width=45, height=15  ,units="cm") 
    print(hp ) 
    dev.off() 
   ...
   bp<- ggplot(subset(mydenames, FDR<FDR.thres) , aes(x = reorder(ensid, logFC) , logFC) )+geom_bar(stat="identity", aes(fill=posneg), position = position_dodge(preserve = 'single'))+coord_flip()  +theme_bw() +ylab (paste("Fold change (FRD <",FDR.thres,")")) +xlab("") + ggtitle( paste( st, " FDR <", FDR.thres, "abs(logFC) >", logFC.thres,  sep=" " ) ) +
theme(legend.position = "none", legend.title=element_blank(),legend.box.background=element_blank(),
      axis.title.x = element_text(face=1, size=20),
      axis.text.x  = element_text( size=20),
      axis.title.y = element_text(face=1, size=20),
      axis.text.y  = element_text(vjust=0.5, size=20), 
      plot.title = element_text(size = 25)) + scale_fill_manual(values= colorbar)   #"#2a1a5e")  ###3c415e")
  png(paste(outdir,"bar.", st, ".png", sep=""), res=300 ,width=30, height=40  ,units="cm")
   print( bp )
   dev.off()   
    #pdf(paste(outdir,"bar.", st, ".pdf", sep=""))
   #print( bp )
   #dev.off()  
...
png(paste(outdir,"set2.volc.png", sep=""), res=300 ,width=35, height=15  ,units="cm")
ggplot(subset(all, set=="set2"), aes(logFC, -1*log(FDR, 10) , colour=col) ) +geom_point( )+facet_grid (.  ~ experiment, drop=TRUE  ) +theme_bw() +geom_vline(xintercept=c(-logFC.thres, logFC.thres), color="grey" ) +geom_hline(yintercept=-1*log(FDR.thres,10), color="grey") +scale_color_manual(values=c("#3c415e" ,"#dfe2e2") ) +geom_text_repel(data=subset(all, -1*log(FDR,10) > -1*log(FDR.thres,10) & set=="set2"  ), aes(label=ensid) )+ theme(legend.position="none", strip.text= element_text(size = 15) )
dev.off()
...
ggplot(subset(all, FDR<FDR.thres & set=="set1") , aes(x = reorder(ensid, logFC) , logFC) )+geom_bar(stat="identity", aes(fill=experiment), position = position_dodge(preserve = 'single'))+coord_flip()  +theme_bw() + scale_fill_manual(values=mycol) +ylab (paste("Fold change (FRD <",FDR.thres,")")) +xlab("")+ theme(legend.title = element_blank())
dev.off() 


png(paste(outdir,"set2.bar.png", sep=""), res=300 ,width=15, height=20  ,units="cm")
ggplot(subset(all, FDR<FDR.thres & set=="set2") , aes(x = reorder(ensid, logFC) , logFC) )+geom_bar(stat="identity", aes(fill=experiment), position = position_dodge(preserve = 'single'))+coord_flip()  +theme_bw() + scale_fill_manual(values=mycol) +ylab (paste("Fold change (FRD <",FDR.thres,")")) +xlab("")+ theme(legend.title = element_blank())
dev.off() 
