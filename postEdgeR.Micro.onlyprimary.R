...
logFC.thres=10
textrepel.thres= 100 #-1*log(FDR.thres,10)
FDR.thres=0.00001
...
for (st in sample_types){ 
    ...
    ##  List of DE genes 
    DEgenes<- mydenames %>% filter (FDR< FDR.thres & abs(logFC)>logFC.thres ) %>% select("ensid")  
    mydheat= merge (DEgenes, mycounts.norm, by="ensid", all.x=F )
    #mydheat$class=sample_name[[mydheat$sample]]
    ...
    print(ggplot(subset(mydenames, FDR<FDR.thres & abs(logFC)>logFC.thres) , aes(x = reorder(ensid, logFC) , logFC) )+geom_bar(stat="identity", aes(fill=experiment), position = position_dodge(preserve = 'single'))+coord_flip()  +theme_bw() + scale_fill_manual(values=mycol) +ylab (paste("Fold change (FRD <",FDR.thres,")")) +xlab("")+ theme(legend.title = element_blank()) +ggtitle( paste(expDir, st, " FDR <", FDR.thres, sep=" " )) )
    dev.off()   
...
ggplot(subset(all, set=="set1"), aes(logFC, -1*log(FDR, 10) , colour=col) ) +geom_point( )+facet_grid (.  ~ experiment, drop=TRUE  ) +theme_bw() +geom_vline(xintercept=c(-logFC.thres, logFC.thres), color="grey" ) +geom_hline(yintercept=-1*log(FDR.thres,10), color="grey") +scale_color_manual(values=c("#3c415e" ,"#dfe2e2") ) +geom_text_repel(data=subset(all, -1*log(FDR,10) > textrepel.thres &  abs(logFC) >logFC.thres  & set=="set1"  ), aes(label=ensid) )+ theme(legend.position="none", strip.text= element_text(size = 15) ) +ggtitle( paste(expDir, st, " FDR <", FDR.thres, sep=" " ))
dev.off()

png(paste(outdir,"set2.volc.png", sep=""), res=300 ,width=45, height=15  ,units="cm")
ggplot(subset(all, set=="set2"), aes(logFC, -1*log(FDR, 10) , colour=col) ) +geom_point( )+facet_grid (.  ~ experiment, drop=TRUE  ) +theme_bw() +geom_vline(xintercept=c(-logFC.thres, logFC.thres), color="grey" ) +geom_hline(yintercept=-1*log(FDR.thres,10), color="grey") +scale_color_manual(values=c("#3c415e" ,"#dfe2e2") ) +geom_text_repel(data=subset(all, -1*log(FDR,10) > textrepel.thres &  abs(logFC)>logFC.thres  & set=="set2"  ), aes(label=ensid) )+ theme(legend.position="none", strip.text= element_text(size = 15) ) +ggtitle( paste(expDir, st, " FDR <", FDR.thres, sep=" " ))
dev.off()
...
ggplot(subset(all, FDR<FDR.thres &  abs(logFC)>logFC.thres & set=="set1" ) , aes(x = reorder(ensid, logFC) , logFC) )+geom_bar(stat="identity", aes(fill=experiment), position = position_dodge(preserve = 'single'))+coord_flip()  +theme_bw() + scale_fill_manual(values=mycol) +ylab (paste("Fold change (FRD <",FDR.thres,")")) +xlab("")+ theme(legend.title = element_blank()) +ggtitle( paste(expDir, st, " FDR <", FDR.thres, sep=" " ))
dev.off() 


png(paste(outdir,"set2.bar.png", sep=""), res=300 ,width=15, height=20  ,units="cm")
ggplot(subset(all, FDR<FDR.thres & abs(logFC)>logFC.thres & set=="set2") , aes(x = reorder(ensid, logFC) , logFC) )+geom_bar(stat="identity", aes(fill=experiment), position = position_dodge(preserve = 'single'))+coord_flip()  +theme_bw() + scale_fill_manual(values=mycol) +ylab (paste("Fold change (FRD <",FDR.thres,")")) +xlab("")+ theme(legend.title = element_blank()) +ggtitle( paste(expDir, st, " FDR <", FDR.thres, sep=" " ))
dev.off() 
